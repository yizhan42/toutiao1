...
public interface MessageDAO {
    ...
    @Insert({"insert into ", TABLE_NAME, "(", INSERT_FIELDS,
            ") values (#{fromId},#{toId},#{content},#{hsaRead},#{conversationId},#{createdDate})"})
    ...
    @Select({"select count(id) from", TABLE_NAME," where has_read=0 and to_id=#{userId} and conversationId=#{conversationId}"})
